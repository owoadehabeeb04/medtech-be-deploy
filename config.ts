...
// Helper to ensure critical variables exist
const required = (key: string): string => {
	const value = process.env[key];
	if (!value) throw new Error(`MISSING CONFIG: ${key} is not defined in .env`);
	return value;
};
...
export const applicationConfig: Configuration = {
	...
	tokenSecret: process.env.JWT_SECRET, 
	refreshTokenSecret: required("REF_JWT_SECRET"), // Guaranteed string or crash
	tokenExpirationTime: Number(process.env.JWT_EXP_TIME) || 900,
	refreshTokenExpirationTime: Number(process.env.REF_JWT_EXP_TIME) || 3600,
	redisExpirationTime: Number(process.env.REDIS_EXP_TIME) || 300,
	bcryptSaltRound: Number(process.env.BCRYPT_SALT_ROUNDS) || 10,
	connectionTimeout: Number(process.env.CONNECTION_TIMEOUT) || 5000,
	rateLimitOptions: {
		duration: Number(process.env.RATE_LIMIT_DURATION) || 60000,
		maxRequestsPerMinute: Number(process.env.RATE_LIMIT_MAX_REQUESTS) || 100,
	},
	mailOptions: {
		username: process.env.MAIL_USER || "",
		password: process.env.MAIL_PASS || "",
	...
	otpExpiration: Number(process.env.OTP_EXPIRATION) || 15,
	url: {
		baseApi: process.env.BASE_API_URL || "http://localhost:6200",
		baseApp: process.env.BASE_APP_URL || "http://localhost:3000",
	},
	googleOauthOption: process.env.GOOGLE_CLIENT_ID
		? {
				clientId: process.env.GOOGLE_CLIENT_ID,
				projectName: process.env.GOOGLE_PROJECT_NAME || "",
				authUri: process.env.GOOGLE_AUTH_URI || "",
