...
const updateNotifications = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { error: validationError, value } = validateSchema(MerchantSettings_schema_1.updateNotificationsSchema, cleanBody);
    if (validationError) {
        return next(manageApplicationErrors({
    ...
    const [error, result] = yield manageAsyncOps(MerchantSettings_service_1.MerchantSettingsService.updateNotifications(merchantId, value));
    if (error) {
        return next(manageApplicationErrors({
