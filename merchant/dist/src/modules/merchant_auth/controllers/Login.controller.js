...
const login = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { error: validationError, value: payload } = validateSchema(MerchantAuth_schema_1.LoginSchema, cleanBody);
    if (validationError) {
    ...
    const [error, data] = yield manageAsyncOps(MerchantAuth_service_1.MerchantAuthService.login(payload, req));
    if (error) {
        return next(manageApplicationErrors({
    ...
    if (!data.status) {
        return next(manageApplicationErrors({
            ...
            statusCode: data.code || http_status_1.UNAUTHORIZED,
        }));
    }
