...
export const verifyResetOtp = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const [error, data] = await manageAsyncOps(MerchantAuthService.verifyResetOtp(payload));
...
/**
 * @swagger
 * /api/v1/merchant/auth/verify-reset-otp:
 *   post:
 *     summary: "Verify reset OTP"
 *     description: "Verify reset OTP for the merchant API."
 *     operationId: "merchant_post_api_v1_merchant_auth_verify_reset_otp"
 *     tags: ["Authentication"]
 *     security: []
 *     requestBody:
 *       required: true
