...
export class DrugstoreMerchantClient {
	private static client: AxiosInstance | null = null;

	private static getClient(): AxiosInstance {
		if (!this.client) {
			const baseUrl = applicationConfig.merchantIntegration?.baseUrl || "";
			const timeout = applicationConfig.merchantIntegration?.timeoutMs || 10000;
	...
	private static headers(method: string, path: string, body?: unknown) {
		const keyId = applicationConfig.merchantIntegration?.keyId || "";
		...
		return {
			"X-Internal-Client": "experience_1",
