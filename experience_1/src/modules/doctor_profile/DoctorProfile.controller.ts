...
export const createDoctorBasicProfile: RequestHandler = async (req, res, next) => {
	const payload = req.context.validateSchema(DoctorBasicProfileSchema, req.context.sanitizeBody(req.body), next);
	if (!payload) return;
...
export const updateDoctorAddress: RequestHandler = async (req, res, next) => {
	const payload = req.context.validateSchema(DoctorAddressSchema, req.context.sanitizeBody(req.body), next);
	if (!payload) return;
...
export const createDoctorEducation: RequestHandler = async (req, res, next) => {
	const payload = req.context.validateSchema(DoctorEducationSchema, req.context.sanitizeBody(req.body), next);
	if (!payload) return;
...
export const createDoctorWorkHistory: RequestHandler = async (req, res, next) => {
	const payload = req.context.validateSchema(DoctorWorkHistorySchema, req.context.sanitizeBody(req.body), next);
	if (!payload) return;
...
export const createDoctorSpecialties: RequestHandler = async (req, res, next) => {
	const payload = req.context.validateSchema(DoctorSpecialtiesSchema, req.context.sanitizeBody(req.body), next);
	if (!payload) return;
